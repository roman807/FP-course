...
//    }
    ...
    def count(char: Char, list: List[Char], current: Int): Int = {
      if (list.isEmpty) current
        ...
        if (char == list.head) count(char, list.tail, current + 1)
        else count(char, list.tail, current)
    }
    def get_unique_chars(chars: List[Char]): List[Char] = {
      if (chars.isEmpty) List()
      else if (isin(chars.head, chars.tail)) get_unique_chars(chars.tail)
      else chars.head :: get_unique_chars(chars.tail)
    }
...
//    println(isort(freqs))
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decode_help(tree: CodeTree, subtree: CodeTree, bits: List[Bit]): List[Char] = subtree match {
      ...
      case Leaf(char, _) =>
        ...
        else char :: decode_help(tree, tree, bits)
    }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    table.find(cell => cell._1 == char).get._2
  }
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def prepend(codetable: CodeTable, bit: Bit): CodeTable = {
      codetable.map(cell => (cell._1, bit :: cell._2))
    }
