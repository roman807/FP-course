...
trait Solver extends GameDef {
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val a = newNeighborsOnly(neighborsWithHistory(initial.head._1, initial.head._2), explored + initial.head._1)
      a ++ from(initial.tail ++ a, explored + initial.head._1)
    }
  ...
  lazy val solution: List[Move] =
    ...
    else pathsToGoal.minBy(p => p._2.length)._2.reverse
