...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    // resulting map: mean -> seq of points
    var mapMeansToPoints = points.groupBy(f => findClosest(f, means))
    for (mean <- means){
      if (!mapMeansToPoints.keys.toList.contains(mean)) mapMeansToPoints += (mean -> Seq.empty[Point])
    }
    mapMeansToPoints
  }
  ...
  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    // resulting map: mean -> seq of points
    var mapMeansToPoints = points.par.groupBy(f => findClosest(f, means))
    for (mean <- means.par){
      if (!mapMeansToPoints.keys.toList.contains(mean)) mapMeansToPoints += (mean -> ParSeq.empty[Point])
    }
    mapMeansToPoints
  }
  ...
  final def kMeans(points: Seq[Point], means: Seq[Point], eta: Double): Seq[Point] = {
    val mapMeansToPoints = classify(points, means)
    val newMeans = update(mapMeansToPoints, means)
